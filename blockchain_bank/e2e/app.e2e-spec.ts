...
describe('Starting tests for blockchain_bank', function() {
  ...
  it('website title should be blockchain_bank', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('blockchain_bank');
    })
  });

  it('network-name should be blockchain_bank@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('blockchain_bank@0.0.1.bna');
    });
  });

  it('navbar-brand should be blockchain_bank',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('blockchain_bank');
    });
  });
